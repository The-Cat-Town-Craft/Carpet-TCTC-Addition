...
public class OperatorCommand {
    ...
    private static int setPermissionLevel(CommandSourceStack commandSourceStack, Collection<GameProfile> gameProfileCollection, int level) {
        if (!commandSourceStack.hasPermission(level)) {
            commandSourceStack.sendFailure(new TextComponent(String.format(tr("carpet-tctc-addition.message.command.operator.setPermissionLevel.lower", "You cannot assign a higher level of permission to a target than you have(level: %d)."), level)));
            return 1;
        }
        for (GameProfile gameProfile : gameProfileCollection) {
            ...
            MessageUtil.sendMessage(commandSourceStack, String.format(tr("carpet-tctc-addition.message.command.operator.setPermissionLevel.success", "Player %s's permissions have been updated to level %d."), gameProfile.getName(), level));
        }
        return 1;
    }
    ...
    private static int setBypassPlayerLimit(CommandSourceStack commandSourceStack, Collection<GameProfile> gameProfileCollection, boolean bypass) {
        for (GameProfile gameProfile : gameProfileCollection) {
            ...
            MessageUtil.sendMessage(commandSourceStack, String.format(tr("carpet-tctc-addition.message.command.operator.setBypassPlayerLimit.success", "Player %s bypassed the player count limit has been updated to %s."), gameProfile.getName(), bypass));
        }
        return 1;
    }
}
