...
public class FixCommand {
    ...
    private static int fixChunk(CommandSourceStack source, ColumnPos pos) throws CommandSyntaxException {
        ...
        MessageUtil.sendMessage(source, (BaseComponent) new TextComponent(String.format(tr("carpet-tctc-addition.message.command.fix.fixing", "Fixing chunk [%s, %s]..."), chunkPos.x, chunkPos.z)).setStyle(new Style().setColor(ChatFormatting.GRAY).setItalic(true)));
        ((ThreadedLevelLightEngine) level.getLightEngine()).lightChunk(levelChunk, false).thenRun(() -> {
            MessageUtil.sendMessage(source, (BaseComponent) new TextComponent(String.format(tr("carpet-tctc-addition.message.command.fix.fixed", "Fixed chunk [%s, %s]"), chunkPos.x, chunkPos.z)).setStyle(new Style().setColor(ChatFormatting.GRAY).setItalic(true)));
        });
        return 1;
    }
}
