...
public class FreecamCommand {
    ...
    public static int executeFreeCamera(CommandSourceStack source, ServerPlayer serverPlayer) {
        ...
        FreeCameraUtil.CameraData cameraDataNew = new FreeCameraUtil.CameraData(serverPlayer.gameMode.getGameModeForPlayer(), serverPlayer.getLevel().dimension(), serverPlayer.getX(), serverPlayer.getY(), serverPlayer.getZ(), serverPlayer.getYRot(), serverPlayer.getYRot(), true);
        if (cameraData == null || !cameraData.isFreeCamera()) {
            ...
            if (CarpetTCTCAdditionSettings.freecamRestoreLocation && cameraData.isFreeCamera() && serverPlayer.isAlive()) {
                serverPlayer.teleportTo(serverPlayer.server.getLevel(cameraData.getResourceKey()), cameraData.getX(), cameraData.getY(), cameraData.getZ(), cameraData.getYRot(), cameraData.getXRot());
            }
            isCameraMode = false;
